...
it easier to fork this. That will also reduce the dependencies because I don't need webpack etc.
This does mean that it will only support 'modern' browsers.
The optional build step is only needed if you want to minify the code, else you can just import ./src/syntaxhighlighter.js directly.
This code is compatible with es6 import only unless you use something like webpack.
I didn't test if this works in node. 
...
```shell
git clone https://github.com/virtualcodewarrior/syntaxhighlighter.git
```
This should give you a working version of the highlighter.
If you want to run tests or build a minified version, you just have to run 
```shell
npm install
```
...
If you have any custom brushes, import them with registerBrush
```html
<script type="module">
...
Every SyntaxHighlighter brush extends `BaseBrush` class defined in [brush-base](https://github.com/virtualcodewarrior/syntaxhighlighter/src/languages/brush-base).
...
Developing a custom brush allows you to easily extend SyntaxHighlighter to support the syntax that isn't [[currently available|Brushes and Themes]]. This process is rather simple and consists of 4 parts:
